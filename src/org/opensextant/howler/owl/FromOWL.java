...
public class FromOWL {
	...
	private List<FactStatement<ObjectPredicate, ProperNoun>> convertOWL(OWLSameIndividualAxiom ax) {
		...
		for (OWLSameIndividualAxiom si : axs) {
			...
			PredicateExpression<ObjectPredicate> pExp = new PredicateExpression<ObjectPredicate>(
					Vocabulary.SAME_Object);
	...
	private List<FactStatement<ObjectPredicate, ProperNoun>> convertOWL(OWLDifferentIndividualsAxiom ax) {
		...
		for (OWLDifferentIndividualsAxiom si : axs) {
			...
			PredicateExpression<ObjectPredicate> pred = new PredicateExpression<ObjectPredicate>(
					Vocabulary.SAME_Object);
	...
	private ProperNoun convertOWLPrimitive(OWLAnonymousIndividual ind) {
		...
		return wordManager.lookupOrCreateByKeyAndType(IRI.create(Vocabulary.ANONYMOUS_NS, normal),
	...
	private Word convertOWLPrimitive(OWLAnnotationSubject subj) {
		...
		LOGGER.error(
				"Couldn't convert AnnotationSubject, not an AnonymousIndividual or IRI. Should not happen." + subj);
		return Vocabulary.ERROR;
	}
	...
	private Word convertOWLPrimitive(OWLAnnotationValue value) {
		...
		LOGGER.error(
				"Couldn't convert AnnotationSubject, not an AnonymousIndividual, Literal or IRI. Should not happen."
						+ value);
		return Vocabulary.ERROR;
	}
	...
	private List<IRI> findBareIRIs(OWLOntology onto) {
		...
		List<OWLAnnotationAssertionAxiom> asserts = OWLAPIStreamUtils
				.asList(onto.axioms(AxiomType.ANNOTATION_ASSERTION));
