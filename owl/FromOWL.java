...
public class FromOWL {
  ...
  // the limit of the number of pairwise axioms to create from a set of entities or -1 to always do all
  private int maxPairs = -1;

  // change a union/intersection/oneOf with a single member to just that member
  ...
  public Document convertOWL(OWLOntology onto) {
    ...
    // renormalize after whole ontology processed to get any labels added during axiom conversion
    wordManager.renormalize();

    // add the vocabulary by lookingup the entities from the ontology signature
    ...
    // add the vocabulary from anonymous individuals (they aren't entities, so not in signature)
  ...
  private boolean includeInVocabulary(OWLEntity entity, OWLOntology onto) {
    ...
    // (NOTE: this only considers LOADED imported ontologies, so if import could not be loaded...)
    ...
    if (onto.isDeclared(entity, Imports.INCLUDED)) {
      ...
      if (!onto.isDeclared(entity, Imports.EXCLUDED)) {
        LOGGER.warn("Not including\t" + entity + "\t" + onto.getOntologyID());
        return false;
      } else {
    ...
    // false if appears in signature of imported onto? Should have been declared in imports?
  ...
  private Phrase convertOWL(OWLObjectIntersectionOf ce) {
    ...
    // if memebers are a set of categories and predicate phrases-> category phrase
  ...
  private DataFacet convertOWLPrimitive(OWLFacet facet) {
    ...
    return DataFacet.getTypeByNormalName(logical);
  }

  private Word convertOWLPrimitive(IRI iri) {
    // any existing word type
    return wordManager.lookupOrCreateByKeyAndType(iri, null);
  }
